...
const SEVERITY_STYLES = {
  critical: { badge: 'bg-red-100 text-red-700 border border-red-200', dot: 'bg-red-500', label: '심각' },
  warning: { badge: 'bg-amber-100 text-amber-700 border border-amber-200', dot: 'bg-amber-500', label: '경고' },
  info: { badge: 'bg-blue-100 text-blue-700 border border-blue-200', dot: 'bg-blue-500', label: '정보' },
}
          ...
          <li key={issue.id} className="flex flex-col gap-1">
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-slate-500 mt-0.5 leading-relaxed">{issue.recommendation}</p>
              </div>
            </div>
          </li>
        )
      })}
      {hidden > 0 && <li className="text-xs text-slate-400 pl-3.5">+{hidden}개 더 있음</li>}
    </ul>
  )
}
